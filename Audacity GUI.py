...
def download_video():
    ...
    try:
        ...
        status_label.config(text=f"Failed to download video. Error: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                filename = f"{yt.title}_{timestamp}.mp3"
                downloader.download(output_path=save_path, filename=filename)
        status_label.config(text=f"Audio '{yt.title}' downloaded successfully.")
    except Exception as e:
        status_label.config(text=f"Failed to download audio. Error: {str(e)}")
...
def download_playlist():
    ...
    try:
        ...
        for video in playlist.videos:
            ...
            x += 1
        status_label.config(text=f"Playlist '{playlist.title}' downloaded successfully.")
    except Exception as e:
        status_label.config(text=f"Failed to download playlist. Error: {str(e)}")
